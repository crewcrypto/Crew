...
const string strMessageMagic = "Crew Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 10) // CREW
    {
        nSubsidy = 8000000 * COIN;
    }
    else if(pindexBest->nHeight < 180000) // 0 Blocks 2880 Blocks daily approx. 2 months approx to secure network 
    {
        nSubsidy = 0 * COIN;
    }
...
return nSubsidy + nFees;
    ...
    else if(pindexBest->nHeight < 10000) // 45k
    {
    nSubsidy = 5 * COIN;
return nSubsidy + nFees;
    }
    else if(pindexBest->nHeight < 20000) // 80k
    {
    nSubsidy = 8 * COIN;
return nSubsidy + nFees;
    }
    else if(pindexBest->nHeight < 30000) // 100k
    {
    nSubsidy = 10 * COIN;
return nSubsidy + nFees;
    }
    else if(pindexBest->nHeight < 40000) // 150k
    {
    nSubsidy = 15 * COIN;
return nSubsidy + nFees;
    }
    else if(pindexBest->nHeight < 80000) // 120k
    {
    nSubsidy = 3 * COIN;
return nSubsidy + nFees;
    }
    else if(pindexBest->nHeight < 180001) // 200k
    {
    nSubsidy = 2 * COIN;
return nSubsidy + nFees;
    }
    else if(pindexBest->nHeight > 180000) // 695k During Crew DpoS for 2 Months After 1 CREW each block 
    {
    nSubsidy = 1 * COIN;
return nSubsidy + nFees;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Crew", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Crew Genesis";
        CTransaction txNew;
        txNew.nTime = 1528622378;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1528622378;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2464762;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x73b173792f761393f1e07abc62384238965c96b070c2c059d892b1feddf9e7d7"));
